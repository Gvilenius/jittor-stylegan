...
def init_linear(linear):
    linear.weight.xavier_gauss_()
    linear.bias.zero_()


def init_conv(conv, glu=True):
    conv.weight.kaiming_normal_()
...
class EqualLR:
    ...
    def compute_weight(self, module):
        ...
        fan_in = (weight.data.shape[1] * weight[0][0].numel())
...
class Blur(nn.Module):
    def __init__(self, channel):
        super().__init__()
        weight = jt.array([[1, 2, 1], [2, 4, 2], [1, 2, 1]], dtype=jt.float32)
    ...
    def execute(self, input, step=0, alpha=-1):
        for i in range(step, -1, -1):
            ...
            if i == 0:
                ...
                out = contrib.concat([out, mean_std], dim=1)
