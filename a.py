...
class EqualLR():
    ...
    def apply(module, name):
        ...
        module.register_parameter((name + '_orig'), jt.array(weight.data))
...
class FusedUpsample(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, kernel_size, padding=0):
        ...
        self.weight = jt.array(weight)
        self.bias = jt.array(bias)
        self.pad = padding
...
class BlurFunctionBackward(Function):
    ...
    @staticmethod
    def execute(ctx, grad_output, kernel, kernel_flip):
...
class BlurFunction(Function):
    ...
    @staticmethod
    def execute(ctx, input, kernel, kernel_flip):
        ctx.save_for_backward(kernel, kernel_flip)
...
class EqualConv2d(nn.Module):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__()
        conv = 
        raise RuntimeError('origin source: <nn.Conv2d(*args, **kwargs)>, There are needed 9 args in Pytorch Conv2d function, but you only provide 2')
        
        raise RuntimeError('origin source: <conv.weight.data.normal_()>, There are needed 3 args in Pytorch normal_ function, but you only provide 0')
        conv.bias.data.zero_()
...
class EqualLinear(nn.Module):
    ...
    def __init__(self, in_dim, out_dim):
        ...
        raise RuntimeError('origin source: <linear.weight.data.normal_()>, There are needed 3 args in Pytorch normal_ function, but you only provide 0')
        linear.bias.data.zero_()
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, kernel_size, padding, kernel_size2=None, padding2=None, downsample=False, fused=False):
        ...
        self.conv1 = nn.Sequential(EqualConv2d(in_channel, out_channel, kernel1, padding=pad1), nn.LeakyReLU(scale=0.2))
        if downsample:
            if fused:
                self.conv2 = nn.Sequential(Blur(out_channel), FusedDownsample(out_channel, out_channel, kernel2, padding=pad2), nn.LeakyReLU(scale=0.2))
            else:
                self.conv2 = nn.Sequential(Blur(out_channel), EqualConv2d(out_channel, out_channel, kernel2, padding=pad2), nn.Pool(2, op='mean'), nn.LeakyReLU(scale=0.2))
        else:
            self.conv2 = nn.Sequential(EqualConv2d(out_channel, out_channel, kernel2, padding=pad2), nn.LeakyReLU(scale=0.2))
...
class NoiseInjection(nn.Module):
    ...
    def __init__(self, channel):
        super().__init__()
        self.weight = jt.array(torch.zeros(1, channel, 1, 1))
...
class ConstantInput(nn.Module):
    ...
    def __init__(self, channel, size=4):
        super().__init__()
        self.input = jt.array(torch.randn(1, channel, size, size))
...
class StyledConvBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, kernel_size=3, padding=1, style_dim=512, initial=False, upsample=False, fused=False):
        ...
        self.lrelu1 = nn.LeakyReLU(scale=0.2)
        self.conv2 = EqualConv2d(out_channel, out_channel, kernel_size, padding=padding)
        self.noise2 = equal_lr(NoiseInjection(out_channel))
        self.adain2 = AdaptiveInstanceNorm(out_channel, style_dim)
        self.lrelu2 = nn.LeakyReLU(scale=0.2)
    ...
    def __init__(self, fused=True, from_rgb_activate=False):
        ...
        def make_from_rgb(out_channel):
            if from_rgb_activate:
                return nn.Sequential(EqualConv2d(3, out_channel, 1), nn.LeakyReLU(scale=0.2))
            else:
    ...
    def execute(self, input, step=0, alpha=(- 1)):
        for i in range(step, (- 1), (- 1)):
            ...
            if (i == 0):
                ...
                mean_std = mean_std.expand(out.shape[0], 1, 4, 4)
                out = jt.contrib.concat([out, mean_std], dim=1)
